...
// Change Electron related folder name to `SiYuan-Electron` under `~/.config` https://github.com/siyuan-note/siyuan/issues/3349
app.setPath("userData", app.getPath("userData") + "-Electron");

// Delete auto-created application directory https://github.com/siyuan-note/siyuan/issues/13150
fs.rmSync(app.getPath("appData") + "/" + app.name, { recursive: true }); // Delete auto-created application directory https://github.com/siyuan-note/siyuan/issues/13150
...
const exitApp = (port, errorWindowId) => {
  ...
  // Close all non-main windows with the same port
...
const initMainWindow = () => {
  ...
  writeLog(
    "window stat [x=" +
      ...
      oldWindowState.y +
      ", width=" +
      oldWindowState.width +
      ", height=" +
      oldWindowState.height +
      "], " +
      "workArea [width=" +
      workArea.width +
      ", height=" +
  ...
  if (workArea) {
    // When window is larger than workArea, minimize will hide it in bottom-left corner, reset using minimum value here
    if (
      windowState.width > workArea.width + 32 ||
      windowState.height > workArea.height + 32
    ) {
      // Window size resets to default after restart https://github.com/siyuan-note/siyuan/issues/7755 https://github.com/siyuan-note/siyuan/issues/13732
      // Here +32 is because in some cases window size is a few pixels larger than workArea causing default reset, +32 avoids this edge case
      windowState.width = Math.min(defaultWidth, workArea.width);
      windowState.height = Math.min(defaultHeight, workArea.height);
      writeLog(
        "reset window size [width=" +
          windowState.width +
          ", height=" +
          windowState.height +
          "]"
  ...
  const currentWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      webSecurity: false,
      contextIsolation: false,
      autoplayPolicy: "user-gesture-required", // Disable auto-play multimedia in desktop https://github.com/siyuan-note/siyuan/issues/7587
    },
  ...
  currentWindow.webContents.userAgent =
    "SiYuan/" +
    appVer +
  ...
  // set proxy
  net
    .fetch(getServer() + "/api/system/getNetwork", { method: "POST" })
    .then((response) => {
      return response.json();
    })
    .then((response) => {
      setProxy(
        `${response.data.proxy.scheme}://${response.data.proxy.host}:${response.data.proxy.port}`,
  ...
  currentWindow.webContents.session.webRequest.onBeforeSendHeaders(
    (details, cb) => {
      if (-1 < details.url.indexOf("bili")) {
        // Do not remove Referer for Bilibili https://github.com/siyuan-note/siyuan/issues/94
  ...
  currentWindow.webContents.on("did-finish-load", () => {
    ...
    if (siyuanOpenURL) {
      ...
      currentWindow.show();
      setTimeout(() => {
        // Wait for interface js to finish executing
  ...
  // Open current page links using browser
...
const initKernel = (workspace, port, lang) => {
  return new Promise(async (resolve) => {
    ...
    const kernelName =
      "win32" === process.platform ? "SiYuan-Kernel.exe" : "SiYuan-Kernel";
    const kernelPath = path.join(appDir, "kernel", kernelName);
    if (!fs.existsSync(kernelPath)) {
      showErrorWindow(
        "⚠️ Kernel program is missing",
        `<div>The kernel program is not found, please reinstall SiYuan and add SiYuan Kernel program into the trust list of your antivirus software.</div><div><i>${kernelPath}</i></div>`
      );
      bootWindow.destroy();
      resolve(false);
      return;
    ...
    if (!isDevEnv || workspaces.length > 0) {
      ...
      const kernelProcess = cp.spawn(kernelPath, cmds, {
        // Desktop kernel process is no longer launched in detached mode https://github.com/siyuan-note/siyuan/issues/6336
        detached: false, // Desktop kernel process no longer launched in detached mode https://github.com/siyuan-note/siyuan/issues/6336
        stdio: "ignore",
      });

      const currentKernelPort = kernelPort;
      writeLog(
        "booted kernel process [pid=" +
          kernelProcess.pid +
          ", port=" +
          kernelPort +
          "]"
      ...
      kernelProcess.on("close", (code) => {
        writeLog(
          `kernel [pid=${kernelProcess.pid}, port=${currentKernelPort}] exited with code [${code}]`
        );
        if (0 !== code) {
          let errorWindowId;
          switch (code) {
            case 20:
              errorWindowId = showErrorWindow(
                ...
                "<div>The database file is being occupied by other processes, please check whether there are multiple kernel processes (SiYuan Kernel) serving the same workspace at the same time.</div>"
              );
              break;
            case 21:
              errorWindowId = showErrorWindow(
                "⚠️ Failed to listen to port " + currentKernelPort,
                "<div>Failed to listen to port " +
                  currentKernelPort +
                  ", please make sure the program has network permissions and is not blocked by firewalls and antivirus software.</div>"
              );
              break;
            case 24: // The workspace is locked, try to switch to the first opened workspace
              if (workspaces && 0 < workspaces.length) {
                showWindow(workspaces[0].browserWindow);
              }

              errorWindowId = showErrorWindow(
                "⚠️ The workspace is locked",
                "<div>The workspace is being used, please try to end the SiYuan-Kernel process in the task manager or restart the operating system and then start SiYuan.</div>"
              );
              break;
            case 25:
              errorWindowId = showErrorWindow(
                "⚠️ Failed to create workspace directory",
                "<div>Failed to init workspace.</div>"
              );
              break;
            case 26:
              errorWindowId = showErrorWindow(
            ...
            default:
              errorWindowId = showErrorWindow(
                "⚠️ The kernel exited for unknown reasons",
    ...
    for (;;) {
      try {
        ...
        if (14 < ++count) {
          ...
          showErrorWindow(
            "⚠️ Failed to get kernel serve port",
            "<div>Failed to get kernel serve port, please make sure the program has network permissions and is not blocked by firewalls and antivirus software.</div>"
          );
          bootWindow.destroy();
          resolve(false);
          return;
    ...
    if (0 === apiData.code) {
      ...
      if (!isDevEnv && apiData.data !== appVer) {
        ...
        while (!progressing) {
          try {
            const progressResult = await net.fetch(
...
app.whenReady().then(() => {
  const resetTrayMenu = (tray, lang, mainWindow) => {
    ...
    if ("win32" === process.platform) {
      // Windows supports window always on top https://github.com/siyuan-note/siyuan/issues/6860
  ...
  const hideWindow = (wnd) => {
    // After minimizing with `Alt+M`, focus returns to previous window https://github.com/siyuan-note/siyuan/issues/7275
    wnd.minimize();
    // Mac cannot show in Dock after hiding
    if ("win32" === process.platform || "linux" === process.platform) {
      wnd.hide();
    }
  };
  ...
  ipcMain.on("siyuan-context-menu", (event, langs) => {
    const template = [
      ...
      new MenuItem({
        role: "pasteAndMatchStyle",
        label: langs.pasteAsPlainText,
      }),
      new MenuItem({
        role: "selectAll",
        label: langs.selectAll,
      }),
    ];
    const menu = Menu.buildFromTemplate(template);
  ...
  ipcMain.handle("siyuan-get", (event, data) => {
    ...
    if (data.cmd === "printToPDF") {
      try {
        return getWindowByContentId(data.webContentsId).webContents.printToPDF(
          data.pdfOptions
        );
      } catch (e) {
    ...
    if (data.cmd === "siyuan-open-file") {
      ...
      BrowserWindow.getAllWindows().find((item) => {
        ...
        const ids = decodeURIComponent(
          new URL(item.webContents.getURL()).hash.substring(1)
        ).split("\u200b");
  ...
  ipcMain.on("siyuan-cmd", (event, data) => {
    ...
    switch (cmd) {
      ...
      case "setTrafficLightPosition":
        ...
        if (
          new URL(currentWindow.getURL()).pathname ===
  ...
  ipcMain.on("siyuan-export-pdf", (event, data) => {
    dialog
      ...
      .then((result) => {
        ...
        getWindowByContentId(data.parentWindowId).send(
          "siyuan-export-pdf",
          data
        );
      });
  ...
  ipcMain.on("siyuan-export-newwindow", (event, data) => {
    ...
    const wndScreen = screen.getDisplayNearestPoint({
      x: wndBounds.x,
      y: wndBounds.y,
    });
    const printWin = new BrowserWindow({
      ...
      webPreferences: {
        ...
        webviewTag: true,
        webSecurity: false,
        autoplayPolicy: "user-gesture-required", // Disable multimedia autoplay on desktop https://github.com/siyuan-note/siyuan/issues/7587
      },
    });
    printWin.center();
    printWin.webContents.userAgent =
      "SiYuan/" +
      appVer +
      " https://b3log.org/siyuan Electron " +
      printWin.webContents.userAgent;
  ...
  ipcMain.on("siyuan-open-window", (event, data) => {
    ...
    const mainScreen = screen.getDisplayNearestPoint({
      x: mainBounds.x,
      y: mainBounds.y,
    });
    ...
    const targetScreen = screen.getDisplayNearestPoint(
  ...
  ipcMain.handle("siyuan-init", async (event, data) => {
    const exitWS = workspaces.find((item) => {
      if (
        event.sender.id === item.browserWindow.webContents.id &&
        item.workspaceDir
      ) {
        if (
          (item.tray && "win32" === process.platform) ||
    ...
    if ("win32" === process.platform || "linux" === process.platform) {
      ...
      tray.setToolTip(
        `${path.basename(data.workspaceDir)} - SiYuan v${appVer}`
      );
      const mainWindow = getWindowByContentId(event.sender.id);
      if (!mainWindow) {
        return;
    ...
    await net.fetch(
      getServer(data.port) + "/api/system/uiproc?pid=" + process.pid,
      { method: "POST" }
    );
  });
      ...
      if (index === 0) {
        globalShortcut.register(shortcut, () => {
          ...
          const currentWebContentsId =
            latestActiveWindow && !latestActiveWindow.isDestroyed()
              ? latestActiveWindow.webContents.id
              : undefined;
          workspaces.find((workspaceItem) => {
            if (
              currentWebContentsId ===
                workspaceItem.browserWindow.webContents.id &&
              workspaceItem.hotkeys[0] === item
            ) {
          ...
          if (mainWindow.isMinimized()) {
            mainWindow.restore();
            mainWindow.show(); // Show window again. Fix: When window was hidden with `Alt+M`, showing it again with `Alt+M` caused it to freeze and become uneditable https://github.com/siyuan-note/siyuan/issues/8456
          } else {
            if (mainWindow.isVisible()) {
  ...
  if (firstOpen) {
    ...
    // Improve the appearance language used during desktop initialization https://github.com/siyuan-note/siyuan/issues/6803
    let languages = app.getPreferredSystemLanguages();
    let language =
      languages && 0 < languages.length && "zh-Hans-CN" === languages[0]
        ? "zh_CN"
        : "en_US";
  ...
  // Power-related events must be placed inside whenReady, otherwise it will cause Linux to fail to start normally with Trace/breakpoint trap (core dumped) https://github.com/siyuan-note/siyuan/issues/9347
  powerMonitor.on("suspend", () => {
    writeLog("system suspend");
  });
  powerMonitor.on("resume", async () => {
    // After the desktop system wakes up from sleep, check network connectivity before executing data synchronization https://github.com/siyuan-note/siyuan/issues/6687
  ...
  powerMonitor.on("shutdown", () => {
    ...
    workspaces.forEach((item) => {
      const currentURL = new URL(item.browserWindow.getURL());
      net.fetch(getServer(currentURL.port) + "/api/system/exit", {
        method: "POST",
      });
    });
  });
